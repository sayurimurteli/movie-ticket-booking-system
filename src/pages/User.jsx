...
const User = () => {
	...
	const [activeTab, setActiveTab] = useState('tickets'); // 'tickets' or 'payments'
		...
		<div className="flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 sm:gap-8">
			...
			<div className="mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
				...
				<div
					...
					style={{ gridTemplateColumns: 'repeat(3, minmax(max-content, 1fr)) max-content max-content max-content' }}
				>
								...
								<Fragment key={index}>
									...
									<div className="border-t-2 border-indigo-200 px-2 py-1">
										<button
											...
											${
												ticketsUser === user.username && activeTab === 'tickets'
									...
									<div className="border-t-2 border-indigo-200 px-2 py-1">
										<button
											className={`flex items-center justify-center gap-1 rounded bg-gradient-to-r py-1 pl-2 pr-1.5 text-sm font-medium text-white disabled:from-slate-500 disabled:to-slate-400
											${
												paymentsUser === user.username && activeTab === 'payments'
													? 'from-indigo-600 to-blue-500 hover:from-indigo-500 hover:to-blue-400'
													: 'from-gray-600 to-gray-500 hover:from-gray-500 hover:to-gray-400'
											}`}
											onClick={() => {
												setPayments(user.payments);
												setPaymentsUser(user.username);
												setActiveTab('payments');
											}}
										>
											View {user.payments?.length || 0} Payments
				...
				{/* Tickets Tab */}
				{ticketsUser && activeTab === 'tickets' && (
					<>
						<h2 className="mt-4 text-2xl font-bold text-gray-900">Viewing {ticketsUser}'s tickets</h2>
						{tickets.length === 0 ? (
							<p className="text-center">This user has not purchased any tickets yet</p>
						) : (
											...
											<div className="flex h-full flex-col justify-between rounded-b-lg bg-gradient-to-br from-indigo-100 to-white text-center text-lg drop-shadow-lg md:flex-row">
												...
												{ticket.paymentId && (
													<div className="flex items-center justify-center gap-2 px-4 py-2 text-sm">
														...
														<span className="text-gray-600">{formatDate(ticket.paymentDate)}</span>
													</div>
												)}
											</div>
										</div>
				...
				{/* Payments Tab */}
				{paymentsUser && activeTab === 'payments' && (
					<>
						<h2 className="mt-4 text-2xl font-bold text-gray-900">Viewing {paymentsUser}'s payment history</h2>
						{payments.length === 0 ? (
							<p className="text-center">This user has no payment history</p>
						) : (
